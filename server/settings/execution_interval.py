...
def get_execution_interval() -> float:
    ...
    Returns:
        스크립트 실행 간격 값 (초 단위, float). 설정이 없으면 0.5초 반환
    """
    try:
        # DB에서 설정 조회
        ...
        if interval_str is None:
            # 설정이 없으면 기본값 0.5초 반환
            logger.debug("[Settings] 스크립트 실행 간격 설정이 없어 기본값 0.5초 사용")
            return 0.5
        ...
        try:
            ...
            if interval <= 0:
                logger.warning(f"[Settings] 잘못된 스크립트 실행 간격 값: {interval}, 기본값 0.5초 사용")
                return 0.5
            logger.debug(f"[Settings] 스크립트 실행 간격 조회 성공: {interval}초")
            return interval
        except (ValueError, TypeError):
            logger.warning(f"[Settings] 스크립트 실행 간격 값 변환 실패: {interval_str}, 기본값 0.5초 사용")
            return 0.5
    except Exception as e:
        logger.error(f"[Settings] 스크립트 실행 간격 조회 중 오류 발생: {e}, 기본값 0.5초 사용")
        return 0.5
