...
export class NodeSettingsModal {
    ...
    async show(nodeElement) {
        ...
        // 배열 파라미터 이벤트 리스너 설정 (더 긴 지연으로 DOM 완전 준비 대기)
        setTimeout(() => {
    ...
    setupEventListeners(nodeElement, nodeId, nodeType, nodeData) {
        ...
        setTimeout(() => {
            ...
            // file_path 또는 _file_path로 끝나는 파라미터의 파일 선택 버튼 찾기
            const allBrowseButtons = document.querySelectorAll('[id$="-browse-btn"]');
            const filePathButtons = Array.from(allBrowseButtons).filter((btn) => {
                const btnId = btn.id.toLowerCase();
                return btnId.includes('file_path') && !btnId.includes('folder_path');
            });
    ...
    async handleFileSelection(fieldId) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error('[NodeSettingsModal] 파일 선택 HTTP 에러:', response.status, errorMessage);
                // 사용자에게는 간단한 메시지만 표시
            ...
            try {
                ...
                if (!responseText || responseText.trim() === '') {
                    console.warn('[NodeSettingsModal] 빈 응답 수신');
                    return;
                }
                result = JSON.parse(responseText);
            } catch (parseError) {
                console.error('[NodeSettingsModal] JSON 파싱 실패:', parseError);
            ...
            if (result.success && result.data?.file_path) {
                ...
                console.log('[NodeSettingsModal] 파일 선택 취소됨');
            } else {
                // 기타 실패 케이스
                const errorMsg = result.message || '파일 선택에 실패했습니다.';
                console.warn('[NodeSettingsModal] 파일 선택 실패:', errorMsg);
                // 취소가 아닌 경우에만 알림 표시
            ...
            console.error('[NodeSettingsModal] 파일 선택 중 예외 발생:', error);
            alert('파일 선택에 실패했습니다. 서버가 실행 중인지 확인하세요.');
        } finally {
            btn.disabled = false;
            btn.textContent = originalText;
    ...
    async checkExcelOpenRequirement(nodeType, nodeId) {
        ...
        const excelNodesRequiringOpen = Object.keys(allConfigs).filter(
            (nodeTypeKey) =>
                nodeTypeKey.startsWith('excel-') && nodeTypeKey !== 'excel-open' && nodeTypeKey !== 'excel-compare'
        );
    ...
    /**
     * 배열 파라미터 이벤트 리스너 설정
     * 이벤트 위임을 사용하여 동적으로 추가되는 요소에도 작동하도록 함
     */
    setupArrayParameterListeners() {
        ...
        log('[NodeSettingsModal] 배열 파라미터 이벤트 리스너 설정 시작');
        ...
        this.arrayParameterClickHandler = (e) => {
            if (e.target.classList.contains('array-item-add') || e.target.closest('.array-item-add')) {
                ...
                log(`[NodeSettingsModal] 배열 항목 추가 버튼 클릭: ${paramKey}, fieldId: ${fieldId}`);

                const container = document.getElementById(`${fieldId}-container`);

                if (!container) {
                    log(`[NodeSettingsModal] 컨테이너를 찾을 수 없음: ${fieldId}-container`);
                    return;
                }
                ...
                log(`[NodeSettingsModal] 배열 항목 추가 완료: ${newIndex}`);
            ...
            if (e.target.classList.contains('array-item-remove') || e.target.closest('.array-item-remove')) {
                ...
                if (itemWrapper) {
                    ...
                    if (container) {
                        ...
                        this.updateArrayParameterValue(fieldId, container);
                        log('[NodeSettingsModal] 배열 항목 삭제 완료');
                    }
                }
            }
        };
        ...
        log('[NodeSettingsModal] 배열 파라미터 이벤트 리스너 설정 완료');
    }
