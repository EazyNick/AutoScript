...
> **⚠️ 중요**: 이 문서는 **미구현 설계 문서**입니다. 현재 코드베이스에서는 이 Wrapper 시스템이 구현되지 않았습니다. 실제 구현은 `BaseNode`와 `@NodeExecutor` 데코레이터를 사용하고 있습니다. 자세한 내용은 [현재 Wrapper 사용 현황](./current-wrapper-usage.md) 문서를 참고하세요.
...
**대상 노드**: `start` (종료 노드는 더 이상 사용되지 않음)
