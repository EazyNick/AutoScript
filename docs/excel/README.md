...
### 2. 기본 워크플로우
1. **엑셀 열기**: Excel 파일을 열고 `execution_id`를 출력으로 반환합니다
2. **엑셀 시트 선택**: 원하는 시트를 선택합니다 (이전 노드 출력에서 `execution_id` 선택)
3. **엑셀 닫기**: 작업 완료 후 Excel을 닫습니다 (이전 노드 출력에서 `execution_id` 선택)

### 3. 엑셀 비교 (종합 노드)
```
[시작] → [엑셀 비교]
         (원본 파일, 대상 파일, 비교할 열, 복사할 열 설정)
```

엑셀 비교 노드는 엑셀 열기부터 닫기까지 모든 작업을 한 노드에서 처리합니다.
...
- `server/nodes/excelnodes/excel_compare.py` - 엑셀 비교 노드
- `server/nodes/excelnodes/excel_close.py` - 엑셀 닫기 노드
- `server/nodes/excelnodes/excel_manager.py` - Excel 인스턴스 관리

### JavaScript UI
- `UI/src/js/components/node/excelnodes/node-excel-open.js`
- `UI/src/js/components/node/excelnodes/node-excel-select-sheet.js`
- `UI/src/js/components/node/excelnodes/node-excel-compare.js`
- `UI/src/js/components/node/excelnodes/node-excel-close.js`
